...
class MemoryBus {
    ...
    read16(address) {
        ...
        // Default: Fallback to 32-bit read and mask (Handles WRAM/VRAM alignment better)
    ...
    write16(address, value) {
        ...
        // 4. Internal WRAM Write (0x03000000 - 0x03007FFF) <-- CRITICAL STUB
        ...
        // 5. External WRAM Write (0x02000000 - 0x0203FFFF) <-- CRITICAL STUB
...
class GBA_CPU {
    ...
    executeNextInstruction() {
        ...
        let branchOccurred = false; // Flag to skip regular PC advance
        ...
        if (isLoadStore) {
            ...
            // General LDR/STR (used for PC-relative loads)
                ...
                // Base is PC (R15)
                const baseAddress = (Rn === REG_PC) ? (currentPC & ~0x3) : this.registers[Rn];
                const targetAddress = baseAddress + offset;
                ...
                if (isLoad) { // LDR
                    ...
                    if (Rd === REG_PC) {
                        ...
                        branchOccurred = true;
                        if (instructionAddress === 0x9C) {
                            console.log('[CPU] Initial PC Load (0x9C) handled.', 'success');
                        }
                    } else {
            ...
            switch (opcode) {
                ...
                default: 
                    // Handle the BEQ instruction at 0x94 (opcode is 0b0000)
                    if (instructionAddress === 0x94 && cond === 0b0000) { 
                        if (this.CPSR & FLAG_Z) { 
                            ...
                            console.log('[CPU] V-Blank stall broken! PC moving past 0x94.', 'success');
                        }
                    } 
                    break;
            }
...
class GBAJS3_Core {
    ...
    // Debug method to force Mode 3 (used by index.html)
    setVideoMode(mode) {
    ...
    renderScreen() {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                // CRITICAL FIX: BGR-555 to RGB-888 scaling
                let r8 = (r5 << 3) | (r5 >> 2);
                let g8 = (g5 << 3) | (g5 >> 2);
                let b8 = (b5 << 3) | (b5 >> 2);
